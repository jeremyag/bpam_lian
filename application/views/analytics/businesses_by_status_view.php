...
<div class="card">
    <div class="card-body">
        <canvas id="businesses_by_status" width="400" height="100"></canvas>
        <script>
            var checked = false;
            function getAnalytics(data)
            {
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        {
                            $("#report").append(
                                "<tr><td>"+data.status[x]+"</td><td>"+data.count[x]+"</td></tr>"
                            );

                            if(!checked)
                            {
                                $("#checkbox-list").append(
                                    "<td class='checkbox'><input class='in-check' type='checkbox' name='includes' value='"+data.status[x]+"' checked='checked'> "+data.status[x]+"</td>"
                                );
                            }
                        }
                        ...
                        var ctx = document.getElementById('businesses_by_status').getContext('2d');
